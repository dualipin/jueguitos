...
<template>
  <div class="min-h-dvh overflow-hidden bg-gradient-to-b from-blue-50 to-purple-100">
    ...
    <div class="max-w-4xl mx-auto p-8 text-center flex-grow relative mt-24 z-10">
      <h1
        class="text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-purple-700 mb-8 animate-title"
      >
      ...
      <div v-if="loading" class="flex justify-center items-center mb-8 animate-pulse">
        ...
        <p class="text-gray-700 ml-4">Cargando compuesto...</p>
      </div>

      <!-- Timer display -->
      <div class="mb-4 flex justify-center">
        <div
          class="text-xl font-mono bg-white/80 px-4 py-2 rounded-lg shadow-md inline-flex items-center gap-2"
          :class="{ 'text-red-600 animate-pulse': timeLeft < 10 }"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="h-5 w-5"
      ...
      <div
        class="bg-white/90 backdrop-blur-sm rounded-2xl shadow-2xl p-8 mb-8 transform transition-all duration-500 hover:shadow-[0_20px_50px_rgba(8,112,184,0.2)] border border-blue-100 animate-card"
      >
        <div class="absolute -right-2 -top-2">
          <div class="bg-blue-500 text-white text-xs px-2 py-1 rounded-full">Compuesto actual</div>
        </div>
        <h2 class="text-3xl font-bold text-gray-800 mb-4">{{ compuesto.nombre }}</h2>
        <div
          v-if="compuesto.estructura"
          class="text-gray-700 italic bg-gradient-to-r from-blue-50 to-purple-50 p-4 rounded-xl border border-blue-100"
        >
      ...
      <div v-if="!isCorrect && timeLeft > 0" class="mb-8 animate-fade-in">
        <p class="text-gray-700 mb-5 text-xl">¿Cuál es la fórmula química de este compuesto?</p>
        <div class="flex flex-col sm:flex-row gap-4 justify-center items-center">
          <input
            ...
            class="border-2 backdrop-blur-2xl bg-white/10 border-blue-200 rounded-lg py-3 px-5 text-gray-700 focus:outline-none focus:ring-4 focus:ring-blue-200 focus:border-blue-500 shadow-sm w-full sm:w-auto text-lg transition-all duration-300"
          />
          <button
            @click="checkAnswer"
        ...
        <div class="mt-6 flex justify-center items-center gap-2">
          <p v-if="attempts > 0" class="text-gray-600 bg-gray-100 rounded-full px-4 py-1">
            Intentos: <span class="font-bold text-blue-600">{{ attempts }}</span>
          </p>
          <p
            v-if="message"
            class="font-bold text-lg px-4 py-1 rounded-full animate-bounce"
            :class="isCorrect ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-500'"
          >
            {{ message }}
          </p>
        </div>
      ...
      <div
        ...
        class="bg-gradient-to-r from-amber-50 to-orange-50 border-2 border-amber-300 rounded-2xl p-8 shadow-2xl animate-success"
      >
        ...
        <p class="text-amber-700 font-bold mb-4 text-2xl">¡Se acabó el tiempo!</p>
        <div class="p-4 bg-white/80 backdrop-blur-sm rounded-xl mb-6 inline-block">
          <p class="mb-2 text-gray-700">La fórmula correcta era:</p>
          <strong class="text-amber-700 text-3xl font-mono tracking-wide">{{
            compuesto.formula
          }}</strong>
        </div>
        <button
      ...
      <div
        v-else
        class="bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-300 rounded-2xl p-8 shadow-2xl animate-success"
      >
        <div class="flex justify-center mb-4">
          <svg
            xmlns="http://www.w3.org/2000/svg"
        ...
        <p class="text-green-700 font-bold mb-4 text-2xl">{{ message }}</p>
        <div class="p-4 bg-white/80 backdrop-blur-sm rounded-xl mb-6 inline-block">
          <p class="mb-2 text-gray-700">La fórmula correcta es:</p>
          <strong class="text-emerald-700 text-3xl font-mono tracking-wide">{{
            compuesto.formula
          }}</strong>
        </div>
        <button
      ...
      <!-- Progress indicator -->
      <div class="mt-8 bg-white/80 backdrop-blur-sm p-4 rounded-xl shadow-md inline-block">
        <p class="text-gray-600">¡Sigue aprendiendo química!</p>
      </div>
    </div>
  </div>
</template>
