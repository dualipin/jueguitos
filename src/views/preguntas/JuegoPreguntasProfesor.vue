...
<template>
  ...
  <div
    v-if="loading"
    class="fixed inset-0 bg-white/80 backdrop-blur-sm z-50 flex justify-center items-center flex-col gap-4"
  >
    <div
      class="animate-bounce bg-blue-600 p-2 w-16 h-16 ring-1 ring-blue-200 shadow-lg rounded-full flex items-center justify-center"
    >
      <svg
    ...
    <span class="text-2xl font-bold text-blue-600">Cargando preguntas...</span>
  </div>
    ...
    <div class="flex flex-col gap-6 w-full max-w-6xl px-4 mx-auto py-8 z-30 min-h-dvh relative">
      ...
      <header class="mb-6">
        <h1 class="flex flex-col sm:flex-row items-start sm:items-center gap-2">
          ...
          <span v-if="profesor" class="text-xl md:text-2xl font-bold sm:ml-2 text-gray-700">
            - Profesor: {{ profesor }}
          </span>
        </h1>
      </header>
      ...
      <div class="backdrop-blur-xs shadow-lg rounded-xl p-6 border border-gray-100 bg-white/50">
      ...
      <!-- Questions section -->
      <div class="backdrop-blur-xs shadow-lg rounded-xl p-6 border border-gray-100 bg-white/50">
        <h2 class="text-2xl md:text-3xl font-bold mb-4 text-gray-800">Preguntas</h2>

        <div v-if="preguntas.length === 0" class="bg-white rounded-lg p-8 text-center shadow-inner">
          <span class="text-lg md:text-xl font-medium text-gray-500">
        ...
        >
          ...
          <span class="text-xl md:text-2xl font-bold text-gray-700">
            {{ correctas }} / {{ preguntas.length }} correctas
          </span>
          <span
            v-if="correctas === preguntas.length"
            class="text-lg md:text-xl font-bold text-green-600 mt-2"
          >
            🎉 ¡Felicidades, completaste todas las preguntas! 🎉
          </span>
          <span v-else class="text-lg md:text-xl font-medium text-blue-600 mt-2">
            ¡Sigue intentando!
          </span>
        </div>
      </div>
